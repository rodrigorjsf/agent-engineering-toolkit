...
- [ ] `description` length ≤ 200 chars (target band 110–150) — measure via Python YAML, not single-line awk; if > 200 chars, frontmatter MUST include `description-budget-exception: <one-line reason>` (absolute hard cap 400 chars). The 200-char ceiling exists because the agent's skill-listing budget (host-platform metadata cap) silently drops descriptions when the catalogue overflows; trimmed descriptions keep skills discoverable.
