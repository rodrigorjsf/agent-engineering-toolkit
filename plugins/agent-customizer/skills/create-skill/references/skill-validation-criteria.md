...
- [ ] `description` length ≤ 200 chars (target band 110–150) — measure via Python YAML, not single-line awk; if > 200 chars, frontmatter MUST include `description-budget-exception: <reason>` (hard cap 400 chars). See `.claude/rules/skill-description-budget.md`, `[[skill-listing-budget]]`, and ADR-0007.
