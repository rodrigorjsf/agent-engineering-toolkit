...
3. **Source documents** (`docs/`) — only when the wiki lacks coverage. Treat `docs/` as raw input: vendor-documentation mirrors (`docs/claude/`, `docs/claude-code/`, `docs/cursor/`) are re-synced against upstream (see ADR-0010); all other `docs/` artifacts are immutable — never hand-edit them.
